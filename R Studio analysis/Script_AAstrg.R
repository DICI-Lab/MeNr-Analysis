...
# Initialize an empty matrix to store the percentage identity values 
n <- length(PROTcons)
pid_matrix <- matrix(PROTcons, nrow = 22, ncol = 22)  #VAR# #Value dependent on the number of sequences analyzed


# Fill the matrix with percentage identity values
for (i in seq_len(length(myAAseqs))) {
  for (j in seq_len(length(myAAseqs))) {
    if (i <= j) {
      # Perform pairwise alignment between sequences i and j
      palign <- pairwiseAlignment(pattern = myAAseqs[i], subject = myAAseqs[j], 
                                  type = "global", substitutionMatrix = "BLOSUM80", 
                                  gapOpening = 3, gapExtension = 1)
      
      # Calculate the percentage identity
      pid_value <- pid(palign, type = "PID1")
      
      # Store the value in the matrix (both [i, j] and [j, i])
...
distMatrix <- dist.alignment(alignment, "identity")
...
plot(phylotree,
     ...
     cex = 0.6,                 #VAR# # Size of the tip labels
     ...
     x.lim = c(-0.02, 0.65),    #VAR# # Limits for the x-axis
     y.lim = c(-0.95, 32.5),    #VAR# # Limits for the y-axis
)
add.scale.bar(length = 0.1, x = 0.5, y = -0.8)
